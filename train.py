...
# for param in model.spa_var.parameters():
#     param.requires_grad = False
# for param in model.spe_var.parameters():
#     param.requires_grad = False
...
def train(epoch):
    ...
    for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), leave=True):
        ...
        if config[config["train_dataset"]]["Normalized_L1"]:
            ...
            total_uncertainty = s_spatial.detach() + s_spectral.detach()  # (B, C, H, W)

            min_uncertainty = total_uncertainty.min()
            s_hat = total_uncertainty - min_uncertainty

            weighted_l1 = s_hat * l1_error
            ...
            min_uncertainty = total_uncertainty.min()
            s_hat = total_uncertainty - min_uncertainty + 1
...
# 主循环
viz = Visdom()
viz.line([0.], [0], win='TRAIN_SAM_28', opts=dict(title='TRAIN_SAM_28', legend=['SAM']))
viz.line([0.], [0], win='TRAIN_PSNR_28', opts=dict(title='TRAIN_PSNR_28', legend=['PSNR']))
#best_sam = 100.0
best_psnr = 0.0
#best_loss = 1000000
for epoch in range(start_epoch, total_epochs):
    ...
    if epoch % config["trainer"]["test_freq"] == 0:
        ...
        viz.line([plt_PSNR], [epoch], win='TRAIN_PSNR_28', update='append')
        time.sleep(0.5)
        
        plt_SAM = metrics["sam"]
        viz.line([plt_SAM], [epoch], win='TRAIN_SAM_28', update='append')
        time.sleep(0.5)
